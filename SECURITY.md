...
If you can make the pipeline emit an unsupported accusation that neither
`python src/eval_agent.py` nor `python finetune/harness_over_scenes.py` catches,
that is the report we most want. The second is the stronger test: it runs the same
mutations over hundreds of independently generated scenes, so a rule that only works
on the demo geometry shows up there.

## Supported versions

There are no tagged releases. Active development during the NVIDIA Open Models
Codefest 2026 is on `feature/ToppenAdvances`; fixes land there and are merged to
`main`. Please report against whichever branch you tested, and say which commit — the
demo path is pinned to the committed synthetic data in `demo_data/`, so the commit
determines the scene entirely.
...
- The fine-tuning corpus in `finetune/` is synthetic, it is filtered by the same
  validator used to score the result, and the result is distillation from a larger
  model rather than evidence of improved reasoning. All three are stated in
  `finetune/README.md`.
- Scenes generated by `finetune/scene_factory.py` are a deliberate stress
  distribution, not a sample of reality: the hard cases are over-represented on
  purpose.
